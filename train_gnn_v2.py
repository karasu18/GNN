...
def pairwise_rank_loss(pred: torch.Tensor, target: torch.Tensor, sample_pairs: int = 4096) -> torch.Tensor:
    n = pred.shape[0]
    if n < 3:
    ...
    return F.softplus(-sign[valid] * p_diff[valid]).mean()
...
def pick_best_threshold(model, X_list, y_list, m_list, edge_index, to_tensor, start, end) -> float:
    ...
    for thr in np.linspace(-0.01, 0.01, 41):
        _, acc, pred_pos, true_pos, ic = evaluate_range(model, X_list, y_list, m_list, edge_index, to_tensor, start, end, float(thr))
        if np.isnan(acc):
            continue
        score = (0.45 * acc) + (0.55 * (0.0 if np.isnan(ic) else ic)) - 0.2 * abs(pred_pos - true_pos)
        if score > best_score:
            best_score = score
            best_thr = float(thr)
    return best_thr
...
def main(args):
    ...
    if not os.path.exists(args.csv):
        raise FileNotFoundError(
            ...
            "或在训练时显式指定: python train_gnn_v2.py --csv <你的csv路径>"
        )
    ...
    for ep in range(cfg.epochs):
        ...
        ema_model = GNNPredictor(input_dim=len(feat_cols), cfg=cfg)
        ema.copy_to(ema_model)

        vmse, vacc, vpred_pos, vtrue_pos, vic = evaluate_range(
            ema_model, X_list, y_list, m_list, edge_index, to_tensor, n_train, n_valid, threshold=0.0
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--reg-weight", type=float, default=0.45)
